...
testmore:
	@ for conf in $$(find conf -type f -name "????.yml" | grep -v -e Mmus -e Btau | $(shufcmd) | head -2); do echo $$conf; genhub-build.py --cfg $$conf --workdir scratch/testmore/ download format datatypes; rm -r scratch/testmore/; done
