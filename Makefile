...
testmore:
	@ for conf in $$(find conf -type f -name "*.yml" | grep -v -- '-ut' | $(shufcmd) | head -2); do echo $$conf; genhub-build.py --cfg $$conf --workdir scratch/ download format; rm -r scratch/; done
