...
testmore:
	@ set -e && for conf in $$(find conf -type f -name "????.yml" | grep -v -e Mmus -e Btau | $(shufcmd) | grep Emex | head -2); do echo $$conf; genhub-build.py --cfg $$conf --workdir scratch/testmore/ download format prepare stats; rm -r scratch/testmore/; done
