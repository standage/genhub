...
testmore:
	@ for conf in $$(find conf -type f -name "*.yml" | grep -e 'Pcan' -e 'Dqua' | grep -v -- '-ut' | $(shufcmd) | head -2); do echo $$conf; genhub-build.py --cfg $$conf --workdir scratch/testmore/ download format; rm -r scratch/testmore/; done
