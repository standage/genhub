...
if [ ! -f "$HOME/local/tgz/master.tar.gz" ]; then
    ...
    wget -P $HOME/local/tgz http://genometools.org/pub/binary_distributions/gt-1.5.7-Linux_x86_64-64bit-complete.tar.gz
    wget -P $HOME/local/tgz https://github.com/standage/AEGeAn/archive/master.tar.gz
fi

# Install GenomeTools
gtdir="$HOME/local/src/gt-1.5.7-Linux_x86_64-64bit-complete"
tar -xzf $HOME/local/tgz/gt-1.5.7-Linux_x86_64-64bit-complete.tar.gz --directory $HOME/local/src
